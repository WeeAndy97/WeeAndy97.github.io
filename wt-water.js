...
(function(){
  ...
  unitedutils:{
    ...
    s1:'<p class="body">A water utility runs the whole <b>water cycle</b>: it abstracts raw water from reservoirs and rivers, treats it, and pipes it to homes; then it collects the wastewater, treats it, and returns it cleanly to the environment. A <b>water and sewerage company (WaSC)</b> does both halves; it is a regional <b>natural monopoly</b>, so it is regulated.</p>'+
       '<p class="body"><b>United Utilities</b> serves the North-West of England, ~7&nbsp;million people. Its revenue is set by <b>Ofwat</b> on a <b>Regulated Asset Base</b> basis: a return on capital at an allowed rate, plus depreciation and an opex allowance, with outcome incentives (ODIs). Revenue is decoupled from volume. What marks out the current period (<b>PR24</b>) is a step-change in capex for storm overflows, leakage, resilience and new supply, which grows the RAB sharply but stretches financing and customer bills.</p>',
    ...
    s4a:'A water utility\'s costs are real: power for pumping, chemicals, sludge, and a large workforce across treatment works and a vast pipe network, so the margin is lower than electricity or gas networks. But against a RAB-based revenue it is still healthy and stable; the swing factor is the capex programme rather than the running cost.',
    wfNote:'Operating cost is power, chemicals, sludge treatment and a large workforce across the cycle, heavier than electricity/gas networks, so the margin is lower. The value drivers are RAB growth and the allowed return; the volume of water barely matters.',
  ...
  amwater:{
    ...
    s4a:'Running water and wastewater across many states carries real power, chemical and workforce costs, but against a large rate base the margin is healthy. The asset to watch is the <b>rate base</b>: it grows with replacement and acquisitions, and each state allows a return on it.',
    wfNote:'Operating cost is power, chemicals, treatment and workforce across many systems, recovered in rate cases. The swing factors for value are the rate base, the allowed ROE and the pace of acquisitions; volume plays no part.',
  ...
  sabesp:{
    ...
    wfNote:'Operating cost is power, chemicals, treatment and a large workforce across a megacity. Privatisation targets efficiency gains; the swing factors are the universalisation capex and the macro rather than the running cost.',
  ...
  sydneywater:{
    ...
    s4a:'Sydney Water carries normal water-utility costs (power, treatment, workforce) plus the energy of running desalination when needed, so the margin is solid but not high. The centre of gravity is the growth-and-resilience capex on which IPART allows a return.',
  ...
  bewg:{
    ...
    s4a:'Operating hundreds of plants carries real energy, chemical and workforce cost, and wastewater treatment is energy-intensive, so the margin is on the lower side. The asset is the <b>concession book</b>: contracted returns over long terms, plus the construction margin on new build.',
