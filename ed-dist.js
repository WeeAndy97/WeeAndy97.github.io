...
(function(){
  ...
  coned:{
    ...
    wfNote:'Operating cost is maintenance of a dense underground network, storm response, metering and customer service, recovered in the rate case. The swing factors for value are the rate base and allowed ROE rather than the units delivered.',
  ...
  enelbr:{
    ...
    s2:'The town network has all the same parts, substation to feeders to transformers to streets, but here it is mostly overhead, dense, and fighting losses. Under a <b>price cap</b> the concessionaire keeps what it saves by cutting losses and running efficiently, so the <b style="color:#0c6b4f">return</b> rises from the assets and the efficiency gains. Drag the levers: the cash flow is regulated and indexed; the risk is the Brazilian discount rate.',
    ...
    s3:'Enel São Paulo earns a <b>regulated tariff</b> set by ANEEL on a price cap, a return on the regulatory asset base, depreciation and an efficient opex allowance, reset at periodic tariff reviews. The dominant operational lever is <b>loss reduction</b> and efficiency, which the cap lets the operator keep between reviews. New growth comes from connections and reinforcement. The investor question is less about the asset than about the <b>discount rate</b>: Brazilian rates and the real.',
    ...
    finNote:'A Brazilian distribution concession is a <b>regulated, indexed cash flow with an efficiency kicker</b>: cut losses and costs and you keep the gain until the next review. The investment debate is the <b>discount rate</b> (Brazilian rates and the real) far more than the asset itself.',
  ...
  ausgrid:{
    ...
    driverLab:'Allowed return', availLab:'Performance', hrK:'Return on RAB', yrS:'return + depreciation + opex',
    preset:'Load Ausgrid',
    try:'<b>Try this:</b> What sets Ausgrid apart is <b>rooftop solar</b> and two-way flows, which create a whole new category of network investment (voltage management, flexibility, batteries). Wind the RAB up to see how that build-out compounds the regulated return. But remember the entry: the consortium paid a <b>premium to RAB</b> in 2016, so the growth has to justify the price.',
    ...
    s4a:'Ausgrid runs a large urban-and-overhead network, so O&M and vegetation, plus a growing job of managing two-way flows, are real costs, but modest against a RAB-based revenue, so the margin is high and stable. The investment case turns on the <b>capex pipeline</b> that the solar/EV transition is driving.',
    wfNote:'Operating cost is network O&M, vegetation and the growing task of managing two-way flows and voltage. Value hinges less on cost than on the pace of RAB growth from the transition build-out, on which the AER allows a return.',
  ...
  dewa:{
    ...
    wfNote:'Operating cost is the maintenance of a modern, automated underground network plus customer operations. Smart metering and automation keep it low; what moves the numbers is the pace of the build-out and the asset base it creates.',
  ...
  csg:{
    ...
    s2:'Dense high-rise cities, a modern underground network, EV chargers everywhere: the south of China is electrifying transport faster than anywhere on earth. The substation and feeders are the regulated assets; at this scale even a thin <b style="color:#0c6b4f">return</b> on the base is an enormous cash flow. Drag the levers; the model is scale rather than price, just like the Chinese transmission grid.',
    driverLab:'Allowed return', availLab:'Performance', hrK:'Return on assets', yrS:'regulated network revenue',
    preset:'Load China Southern',
    try:'<b>Try this:</b> the allowed return on a <b>state-backed</b> Chinese distributor is <b>thin</b>, but the asset base is vast and growing fast, so the absolute return is huge. The model is scale and electrification rather than price. The cost of capital is very low because the owner is the state.',
    s3:'CSG\'s distribution earns a <b>regulated network charge</b> within China\'s framework, a return on a colossal asset base, depreciation and an opex allowance. <b>Scale and capex</b> do the work: urbanisation and the fastest EV roll-out on earth keep the asset base growing. The allowed return is thin, but applied to a vast, fast-growing base and financed at a state cost of capital, it still adds up to a large, stable cash flow.',
    ...
    s4a:'At this scale the cost base is immense in absolute terms, running distribution for 100m accounts across five provinces, but small relative to the revenue from a colossal asset base, so the margin is healthy. The figure that matters is the <b>capital</b> of urbanisation and EV build-out, on which the state allows a regulated return at a very low cost of capital.',
