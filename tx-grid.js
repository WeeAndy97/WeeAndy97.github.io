...
(function(){
  ...
  itc:{
    ...
    s4a:'Like any transmission owner ITC runs lean: the network is maintained rather than operated like a power plant, so the margin is high and the cash flow stable. The asset to watch is the <b>rate base</b>: it grows with every prudent capital project, and FERC allows a return on it, so the value compounds with the build programme.',
  ...
  cteep:{
    ...
    finNote:'A Brazilian transmission concession is a <b>bond in disguise</b>: a fixed, inflation-indexed availability payment for decades, with negligible volume risk. The entire investment debate is the <b>discount rate</b> (Brazilian real rates and the currency) rather than the asset, which is about as safe as infrastructure gets.',
    ...
    breakers:['<b>The RAP</b>: what was locked at auction, and its inflation indexation, is the cash flow.','<b>Country &amp; currency</b>: Brazilian real rates and the currency set the discount rate; nothing operational comes close.','<b>New auction lots</b>: winning greenfield concessions is the main growth lever (at competitive, thin returns).','<b>Availability</b>: outage penalties are the only real operational risk to the RAP.'],
  ...
  saudi:{
    ...
    finNote:'A state transmission monopoly is a <b>strategic asset first</b>: it earns a regulated return at a low, sovereign-backed cost of capital, and its purpose is to deliver the national energy strategy. The flip side is state rather than financial ownership, lighter disclosure, and returns set by policy.',
  ...
  stategrid:{
    ...
    try:'<b>Try this:</b> the allowed return on a <b>state-backed</b> Chinese network is <b>thin</b> by Western standards, but the asset base is vast, so the absolute return is huge. As with the Chinese ports and airports, the model is scale rather than the percentage. The cost of capital is very low because the owner is the state.',
    ...
    breakers:['<b>Scale &amp; asset base</b>: a thin return on a vast base is the model; growth comes from UHV and renewable integration.','<b>Cost of capital</b>: state backing keeps it very low, lifting the value of even a thin allowed return.','<b>State direction</b>: policy sets strategy and the allowed return; shareholders do not.','<b>Renewable integration</b>: connecting the world\'s largest wind and solar build-out drives the forward capex.'],
