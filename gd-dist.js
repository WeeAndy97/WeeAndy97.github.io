...
(function(){
  ...
  cadent:{
    ...
    s3:'Cadent earns a <b>regulated allowed revenue</b> recovered through distribution charges, on the RIIO-GD2 building blocks. Much of the capex is <b>mandated mains replacement</b>, which grows the RAB and keeps the return ticking even if gas demand falls. The decisive investor question is not this year\'s flow but the <b>long-run role of the network</b> (hydrogen conversion vs electrified heating) and the terminal value at the end of the asset\'s life.',
    ...
    s4a:'A GDN\'s costs are mains maintenance, the gas-emergency service (the 24-hour leak response), and a customer-facing overhead, all modest against a RAB-based revenue, so the margin is high and stable. The real "question" in the case is the asset\'s long-run life rather than opex.',
    wfNote:'Operating cost is mains maintenance, the statutory gas-emergency service, leakage management and customer service, modest against a RAB-based revenue. The margin is high. Value swings on the allowed return and the long-run role of gas; throughput hardly matters.',
    ...
    calcNote:'A working model of a <b>RAB-regulated GDN</b>. The build/entry cost is the RAB, the unlevered return tracks the <b>allowed WACC</b>, and growth is modest (mains replacement, not demand). Note the <b>lower exit multiple</b> than electricity; that discount is the price of terminal-value risk on gas.',
  ...
  socalgas:{
    ...
    s2:'The town network is the same (city gate, mains, governors, homes); what changes is the regulation: under US cost-of-service, revenue recovers the <b>rate base × allowed return</b>. The <b style="color:#0c6b4f">return</b> rises from the assets. Drag the levers to see how a high allowed return drives investment, against the strongest electrification headwind in the country.',
    ...
    s3:'SoCalGas earns a <b>state-regulated allowed revenue</b> set in rate cases: a return on rate base at the allowed ROE, plus depreciation and opex. The model rewards <b>investing</b>: safety, pipeline replacement and integrity management each put dollars into the rate base. The case is rate-base growth at a high ROE, set against California\'s building-electrification policy and the long-run decline question.',
    ...
    s4a:'A vast urban gas network carries real O&M, safety and integrity costs, plus a large customer operation, but against a rate-base revenue the margin is healthy. Everything turns on the <b>rate base</b>: it grows with safety and replacement capital, and California allows a return on it.',
    ...
    calcNote:'A working model of a <b>cost-of-service gas IOU</b>. The allowed return is higher than a European GDN, growth reflects safety/replacement capex, and the exit multiple sits lower to reflect California\'s electrification headwind.',
  ...
  comgas:{
    ...
    s2:'The parts are familiar: city gate, mains, governors, homes. Here, though, the network is <b>expanding</b>, connecting new customers across a growing city. The <b style="color:#0c6b4f">return</b> rises from a growing asset base; drag the levers. The upside is connections; the risk is the Brazilian discount rate.',
    driverLab:'Allowed return', availLab:'Efficiency gain', hrK:'Return on assets', yrS:'regulated tariff revenue',
    preset:'Load Comgás',
    try:'<b>Try this:</b> unlike a Western GDN, Comgás <b>grows the RAB by connecting customers</b>; penetration is still low. Wind the asset base up to see the growth compound. But push the cost of debt: Brazilian rates take a large slice out of the high nominal return.',
    s3:'Comgás earns a <b>regulated tariff</b> set by ARSESP on a price cap: a return on the regulatory asset base, depreciation and an efficient opex allowance, reset at tariff reviews. Growth is structural: <b>new connections</b> across a low-penetration market add to the base. Efficiency gains are kept between reviews. The investor question is the <b>discount rate</b> (Brazilian rates and the real) more than the asset, which is a growth story rather than a decline one.',
    ...
    calcNote:'A working model of a <b>growth gas concession</b>, on an enterprise-value basis. The base grows with connections, the floor is high (regulated/indexed), but the cost of debt is high to reflect Brazilian rates, so much of the strong nominal return is given back in the discounting.',
  ...
  agig:{
    ...
    s1:'<p class="body"><b>Australian Gas Networks</b> (part of AGIG) distributes gas across South Australia and other states. It is a regulated monopoly under the <b>Australian Energy Regulator (AER)</b>, which sets a <b>revenue cap</b> on a RAB basis, with the same building blocks as a UK GDN.</p>'+
    ...
    s2:'City gate, mains, governors, homes: the standard town network, except that some of the gas here is <b>blended hydrogen</b> (the green markers). The <b style="color:#0c6b4f">return</b> rises from the regulated asset base; drag the levers to see a regulated gas monopoly betting on hydrogen to protect its future.',
    driverLab:'Allowed return', availLab:'Performance', hrK:'Return on RAB', yrS:'return + depreciation + opex',
    preset:'Load Australian Gas Networks',
    try:'<b>Try this:</b> the regulated mechanics are the same as a UK GDN\'s: RAB × allowed return. The differentiator is <b>hydrogen blending</b>: if the network can decarbonise, the terminal value holds up; if heating electrifies instead, the exit multiple compresses. The long-run case rests on that option.',
    ...
    s4a:'AGN runs a regulated network lean, so the margin is high and stable. Alongside the steady RAB sits the <b>hydrogen programme</b>: modest capex today, but a potentially decisive option on the asset\'s long-run value.',
  ...
  egypt:{
    ...
    s1:'<p class="body">Where Western gas distribution debates decline, Egypt is doing the opposite: a <b>state-led programme</b> to connect tens of millions of homes to piped natural gas for the first time, replacing bottled LPG. Operators such as <b>Town Gas</b> build the city networks, city gates, mains, governors and service connections, across Cairo and beyond.</p>'+
       '<p class="body">The economics are <b>connection-driven growth</b>: revenue comes from a regulated distribution tariff plus connection fees, on an asset base that is expanding fast as new districts are reticulated. It is a developing-market story (high nominal growth, a low base, real country and currency risk) and a reminder that, globally, gas distribution is still being built out as well as questioned.</p>',
    ...
    try:'<b>Try this:</b> this is the <b>opposite of decline</b>: wind the asset base up and the connection-driven growth compounds. But push the cost of debt: discounted at emerging-market rates, the high nominal Egyptian return shrinks fast, and currency is a real risk.',
    s3:'The network earns a <b>regulated distribution tariff</b> plus <b>connection fees</b>, on an asset base that grows rapidly as new homes are connected. The story is growth rather than terminal value: penetration is rising from a low base. The investor question is the <b>discount rate</b> (Egyptian rates and the pound) and the durability of the state programme, against a long runway of new connections.',
    ...
    s4a:'Building a network from scratch (laying mains and connecting homes) is capital-intensive, and developing-market operating costs are real, so the margin is lower than a mature Western GDN. But the base is compounding fast, and connection fees help fund the build.',
    wfNote:'Operating cost is network O&M, the build-and-connect operation and customer service in a developing market. The margin is lower, but the asset base, and the revenue with it, is growing fast.',
    ...
    calcNote:'A working model of a <b>developing-market gas roll-out</b>, on an enterprise-value basis. Growth is high (connections), the base compounds, but the cost of debt is high to reflect Egyptian rates, and EM discounting takes a big bite out of the headline return.',
  ...
  enn:{
    ...
    s4a:'Running hundreds of city networks plus a gas-sales business carries real cost (procurement, O&M, customer operations), but scale and the sales margin keep the blended margin healthy. What sets it apart is the mix of <b>regulated network</b> plus <b>commercial gas sales</b> and connections.',
