...
(function(){
  ...
  ontario:{
    ...
    facts:[['P3 / DBFM','Structure','design·build·finance·maintain'],['Consolidated court','Asset','combined courthouse'],['Availability','Payment','contracted, indexed'],['No demand risk','Revenue','availability-based'],['Investment-grade','Offtaker','province of Ontario'],['Long concession','Term','25-30 years']],
    s2:'The courthouse is a dignified civic building: windows lit, the columned portico open, the scales over the door, the FM entrance busy. <b>Infrastructure Ontario</b> pays the <b style="color:#0c6b4f">availability payment</b> for keeping it available and maintained. The consortium\'s money is the <b>contracted payment on every courtroom kept available</b>, indexed; the cases heard never enter the calculation. The cost draining out is <b>facilities management and lifecycle</b>. Drag the courtrooms, the charge and the availability to work a core, government-backed annuity.',
  ...
  chile:{
    ...
    s2:'This time the courthouse is a new public justice building: windows lit, the portico open, the scales over the door, the FM entrance busy with a maintenance van. The <b>justice ministry</b> pays the <b style="color:#0c6b4f">availability payment</b> for keeping it available and the FM delivered. The concessionaire\'s money is the <b>contracted payment on every courtroom kept available</b>, indexed, whatever the caseload. Drag the courtrooms, the charge and the availability; together they price a contracted annuity at an emerging-market discount rate.',
    driverLab:'Charge / room', availLab:'Availability', hrK:'Availability payment', yrS:'rooms × charge × availability',
    ledge:{a:'+ availability',b:'+ indexation',c:'− FM'}, demandLabel:'AVAILABILITY',
    ...
    finNote:'A justice-facility concession is a <b>fully contracted government annuity</b>: an indexed payment for keeping the courthouse available, with no demand risk. The investment debate is really about the <b>discount rate</b> (Chilean sovereign risk and the dollar-peso mix) rather than the contract.',
  ...
  australia:{
    ...
    facts:[['PPP','Structure','finance·build·maintain'],['Justice precinct','Asset','courts &amp; tribunals'],['Availability','Payment','contracted, indexed'],['No demand risk','Revenue','tied to availability'],['Investment-grade','Offtaker','state government'],['Long term','Concession','25-30 years']],
    s2:'The precinct below is a modern court complex: windows lit, the portico open, the scales over the door, the FM entrance busy. The <b>state justice department</b> pays the <b style="color:#0c6b4f">availability payment</b> for keeping the building available and maintained. The SPV\'s money is the <b>contracted payment on every courtroom kept available</b>, indexed; case volumes sit with the state. Drag the courtrooms, the charge and the availability to shape a core, government-backed annuity.',
    driverLab:'Charge / room', availLab:'Availability', hrK:'Availability payment', yrS:'rooms × charge × availability',
    ledge:{a:'+ availability',b:'+ indexation',c:'− FM'}, demandLabel:'AVAILABILITY',
    preset:'Load Australian PPP',
    try:'<b>Try this:</b> raise the <b>availability</b> and watch the payment sit flat at the contracted level: the state pays for keeping the building <b>available and maintained</b> rather than for the case activity that sits with the justice department. With an investment-grade state offtaker the discount rate is low and gearing high. Drop availability and deductions bite; that is a clean, contracted infrastructure cash flow at work.',
    s3:'The SPV earns a <b>contracted availability payment</b> for keeping the justice precinct available and maintained to standard, indexed to inflation, with <b>deductions</b> for unavailability. The availability core has <b>no demand risk</b>; court operations and case volumes sit with the state rather than with the FM/availability SPV. So the infrastructure income is a clean, fully contracted government annuity backed by an <b>investment-grade</b> state, set by the courtrooms, the charge and availability.',
    ...
    s4a:'The SPV carries <b>full facilities management and lifecycle</b> over the term, security, cleaning, building services plus plant renewal, sized so the availability payment covers it at a contracted margin. A courthouse is light to run, and with an investment-grade state offtaker the margin is steady. What carries the deal is the <b>contracted, indexed payment</b> and the low discount rate a state government permits, far more than the cost line.',
    wfNote:'Operating cost is the through-life facilities management and lifecycle of a civic building plus soft FM, sized against a contracted payment. With deductions for unavailability, keeping the building available is the operational job; the value sits in the indexed, government-backed payment, while the case activity stays with the state.',
    ...
    breakers:['<b>Availability</b>: the payment is for keeping the building available; deductions bite for unavailability.','<b>State offtaker</b>: an investment-grade counterparty keeps the discount rate low and gearing high.','<b>Indexation</b>: the inflation-linked payment protects the real return over a long term.','<b>Case volumes</b>: court activity sits with the state, so the availability SPV carries no demand risk.'],
  ...
  gulf:{
    ...
    lede:'A Gulf <b>justice complex PPP</b>: a private consortium designs, builds, finances and maintains a flagship court complex, and the justice ministry pays a contracted, government-backed availability charge that carries currency considerations.',
    s1:'<p class="body">Gulf states have begun to deliver social infrastructure, including flagship <b>justice complexes</b>, through availability <b>PPPs</b>. A private consortium <b>designs, builds, finances and maintains</b> the court complex and provides the FM services, while the <b>justice ministry</b> keeps court operations public and pays a contracted <b>availability charge</b> over a long concession, typically backed by the government.</p>'+
       '<p class="body">It is the availability PPP at <b>flagship scale</b>, with an emerging-market twist. The consortium takes <b>no demand risk</b> (case volumes are the state\'s) and earns a contracted, indexed <b>availability payment</b> for keeping the complex available and the FM delivered, with <b>deductions</b> for failures. The questions that dominate are <b>FX and sovereign credit</b>: the payments may be hard-currency-linked and underpinned by a strong-but-emerging sovereign, so the credit and currency story drives the discount rate. A new block extends the complex. (The flag and figures here are illustrative.)</p>',
    ...
    s2:'A flagship court building dominates the scene: windows lit, a grand columned portico, the scales over the door, a busy FM/services entrance. The <b>justice ministry</b> pays the <b style="color:#0c6b4f">availability charge</b> for keeping it available and the FM delivered, government-backed. The consortium\'s money is the <b>contracted payment on every courtroom kept available</b>, indexed. Drag the courtrooms, the charge and the availability; what you are weighing is a contracted annuity with FX/sovereign considerations.',
    ...
    s4a:'The consortium carries <b>full facilities management and lifecycle</b> across a flagship complex, FM services plus the renewal of building stock and plant, sized into the availability charge. A courthouse is light to run, so the margin is healthy, but the swing factor is the <b>contracted payment, the FX linkage and the sovereign credit</b> behind it rather than the cost line.',
    wfNote:'Operating cost is the FM across a flagship complex plus the lifecycle renewal of building stock; the cost is real because deductions penalise failures. The payment is set to cover it; the value driver is the indexed, often FX-linked payment and the sovereign credit behind the government backing.',
    ...
    finNote:'A Gulf justice-complex PPP is a <b>sovereign-backed contracted annuity</b>: an indexed, often FX-linked availability charge for keeping a flagship complex available, no demand risk. The investment case turns less on the contract than on the <b>FX and sovereign credit</b> behind the government backing.',
  ...
  china:{
    ...
    s2:'Below sits a large public court building: many windows lit, a grand portico, the scales over the door, a busy FM/services entrance. The <b>judicial authority</b> pays the <b style="color:#0c6b4f">contracted government payment</b> for keeping it available and the services delivered. The partner\'s money is the <b>contracted payment on every courtroom kept available</b>, indexed and government-backed. Drag the courtrooms, the charge and the availability and watch a large contracted annuity build at a low state cost of capital.',
